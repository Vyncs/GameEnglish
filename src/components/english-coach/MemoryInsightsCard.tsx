  ...
  if (loading && !memory) {
    return (
      <div className="rounded-3xl border border-line bg-surface p-4 shadow-sm">
        <div className="text-xs text-tertiary">Carregando memória pedagógica…</div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            onClick={onAnalyze}
            disabled={analyzing}
            className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-md bg-surface text-violet-700 border border-violet-200 hover:bg-surface transition-colors disabled:opacity-60"
          ...
          <button
            type="button"
            onClick={onReset}
            className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-md bg-surface text-tertiary border border-line hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
      ...
      {expanded && (
        <div className="mt-3 space-y-2.5">
          ...
          {memory.conversationHistorySummary && (
            <div className="rounded-xl bg-surface border border-line px-2.5 py-1.5">
              <div className="text-[10px] uppercase tracking-wider font-semibold text-tertiary mb-0.5">
                Onde paramos
              </div>
              <p className="m-0 text-[11px] text-secondary leading-relaxed">
                {memory.conversationHistorySummary}
              </p>
            </div>
          )}
...
function Section({
  ...
  return (
    <div>
      <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider font-semibold text-secondary mb-1">
        {icon}
        {label}
      </div>
