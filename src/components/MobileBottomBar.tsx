        ...
        {items.map((item) => {
          ...
          if (item.isFab) {
            return (
              ...
              >
                <div className={`relative w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl transition-all duration-200 ${
                  isActive
                  ...
                  {item.badge && (
                    <span className="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 flex items-center justify-center px-1 bg-red-500 text-white text-[10px] font-bold rounded-full shadow-lg animate-pulse">
                      {item.badge}
                    </span>
                  )}
                </div>
                <span className={`text-[10px] font-semibold mt-1 ${isActive ? 'text-violet-600' : 'text-slate-500'}`}>
                  {item.label}
                </span>
              </button>
            );
